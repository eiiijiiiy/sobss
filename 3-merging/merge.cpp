...
double computing_merging_cost_of_one_pair(
    const Nef_Polyhedron& merged_n,
    const Nef_Polyhedron& new_vol)
{
    Nef_Polyhedron diff;
    try
    {
        diff = new_vol - merged_n;
    }
    catch(const std::exception& e)
    {
        // CGAL::draw(new_vol);
    ...
    return computing_volume_of_nef_polyhedron(diff);
}
    ...
    vector<size_t>& pairs, double sigma = 1.)
{
    ...
    for (auto p : Z1_pairs)
    {
        ...
        merging_cost(i, j) = computing_merging_cost_of_one_pair(merged_for_each_row[i], new_vol);
        merging_cost(j, i) = computing_merging_cost_of_one_pair(merged_for_each_row[j], new_vol);
...
int main(int argc, char* argv[]){
    ...
    vector<double> sigmas {1/8.0};
    vector<double> lambdas {1/16.0};
    ...
    for (auto sigma : sigmas)
    {
        ...
        for (size_t i = 0; i < sample_names.size(); i++)
        {
            ...
            double total_volume = precomputing_merging_cost(
                input_vol_params, num_box, merging_cost, all_8pts_vol,
